...
import { getRecentDrinkLogsWithTeam } from '@/lib/prisma/fetchers/drink-log-fetchers'
import { TeamBadge } from '@/components/teams/team-badge'
import { deleteDrinkLogAction } from '../actions'
import { Container } from '@/components/layout/container'

type SuperadminDrinkLogsPageProps = {
...
const manageStatusLabels: Record<string, string> = {
  'drink-log-deleted': 'Drink log deleted successfully.',
}

const manageErrorLabels: Record<string, string> = {
  'schema-required': 'Drink log management requires the multi-event schema.',
  'missing-drink-log': 'Drink log record was not found for the selected event.',
  'delete-drink-log-failed': 'Failed to delete drink log.',
}

export default async function SuperadminDrinkLogsPage({ searchParams }: SuperadminDrinkLogsPageProps) {
  await connection()
  ...
  const drinkLogs = managedEvent ? await getRecentDrinkLogsWithTeam(150, managedEvent.id) : []
  ...
  return (
    <Container size="lg" className="py-6 md:py-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-foreground">Manage Drink Logs</h1>
        <p className="text-muted-foreground mt-2">
          Delete individual drink log entries for {managedEvent?.name ?? 'the selected event'}.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        {drinkLogs.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {drinkLogs.map((drinkLog) => (
              <article key={drinkLog.id} className="p-4 md:p-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-2 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                      <span className="font-bold text-foreground">{drinkLog.user.name}</span>
                      {drinkLog.user.team && <TeamBadge team={drinkLog.user.team} className="text-xs" />}
                      <span className="text-xs text-muted-foreground">
                        {formatDistanceToNow(new Date(drinkLog.createdAt), { addSuffix: true, locale: sl })}
                      </span>
                    </div>

                    <p className="text-foreground break-words">
                      {drinkLog.drinkType} &middot; {drinkLog.points} {drinkLog.points === 1 ? 'point' : 'points'}
                    </p>

                    <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                      <span className="font-mono">Drink Log ID: {drinkLog.id}</span>
                    </div>
                  </div>

                  <form action={deleteDrinkLogAction}>
                    <input type="hidden" name="drinkLogId" value={drinkLog.id} />
