    ...
    <div className="container mx-auto p-4 md:p-8 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-8">
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl md:text-3xl font-bold text-foreground break-words">{category.title}</h1>
          {category.description && <p className="text-base text-muted-foreground mt-2">{category.description}</p>}
        </div>
        <Badge variant={status.variant} className="self-start text-sm px-3 py-1.5">
          {status.label}
        </Badge>
      </div>
      ...
      {questions.length > 0 && (
        <div className="space-y-4 mb-8">
          ...
          {questions.map((q) => (
            <div key={q.id} className="bg-card text-card-foreground border-2 border-border rounded-xl p-5">
              <div className="flex items-center gap-2 mb-2">
                <Badge variant="outline" className="text-sm px-2.5 py-1">
                  #{q.questionNumber}
                </Badge>
                <span className="text-sm text-muted-foreground">{q.questionType === 'numeric' ? 'Numerično' : 'Tekstovno'}</span>
              </div>
              <p className="font-bold text-lg leading-snug">{q.questionText}</p>
              {q.correctAnswer && (
                <p className="text-base text-primary mt-2">Odgovor: {q.correctAnswer}</p>
              )}
      ...
      {category.status === 'draft' && (
        <form action={startCategoryAction.bind(null, category.id)}>
          <button
            type="submit"
            className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90 active:bg-primary/80 text-lg font-bold py-5 px-8 rounded-xl transition-colors flex items-center justify-center gap-3 min-h-[56px]"
          >
      ...
      {category.status === 'active' && (
        <div className="space-y-8">
          ...
          <form action={calculateAndSaveResultAction} className="space-y-6">
            ...
            {questions.map((q) => (
              <div key={q.id} className="bg-card text-card-foreground border-2 border-border rounded-xl p-5">
                ...
                <select
                  ...
                  className="w-full border-2 border-input rounded-xl px-5 py-4 text-base min-h-[56px] bg-background text-foreground"
                  defaultValue=""
                >
            ...
            <label className="flex items-center gap-4 bg-card border-2 border-border rounded-xl p-5 cursor-pointer active:bg-accent/10">
              <input type="checkbox" name="hasNumericBonus" id="numericBonus" className="w-6 h-6 rounded accent-primary" />
            ...
            <button
              type="submit"
              className="w-full bg-primary text-primary-foreground hover:bg-primary/90 active:bg-primary/80 text-lg font-bold py-5 px-8 rounded-xl transition-colors flex items-center justify-center gap-3 min-h-[56px]"
            >
      ...
      {latestResult && (category.status === 'active' || category.status === 'completed') && (
        <div className="mt-8 space-y-6">
          ...
          <div className="bg-card text-card-foreground border-2 border-border rounded-xl p-5 md:p-6">
              ...
              {[latestResult.question1Winner, latestResult.question2Winner, latestResult.question3Winner].map((winnerId, i) => {
                ...
                return (
                  <div key={i} className="text-center p-4 bg-muted rounded-xl">
                    <div className="text-sm text-muted-foreground mb-1">Vprašanje {i + 1}</div>
                    <div className={`font-bold text-base ${user ? 'text-foreground' : 'text-muted-foreground italic'}`}>
                      {user ? user.name : 'Brez zmagovalca'}
                    </div>
                    {user?.team && <div className="text-sm text-muted-foreground">{user.team.name}</div>}
                  </div>
                )
              })}
            </div>
            ...
            {/* Scenario & points */}
            <div className="bg-accent/20 border-2 border-accent/50 rounded-xl p-5 mb-4">
              <div className="flex items-center gap-3 mb-3">
                <Trophy className="h-6 w-6 text-primary shrink-0" />
                <span className="font-bold text-xl text-foreground">{SCENARIO_LABELS[latestResult.scenario] || latestResult.scenario}</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-base">
                <div>
                  <span className="text-muted-foreground">Osnovne točke:</span>
                  <span className="font-bold ml-2 text-foreground">{latestResult.basePoints}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Numerični bonus:</span>
                  <span className="font-bold ml-2 text-foreground">{latestResult.numericBonus}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Skupaj:</span>
                  <span className="font-bold ml-2 text-primary text-lg">{latestResult.finalPoints}</span>
                </div>
              </div>
              {latestResult.categoryWinner && (
                <div className="mt-4 pt-4 border-t-2 border-border">
                  <span className="text-muted-foreground text-base">Zmagovalec kategorije: </span>
              ...
              {latestResult.manualOverride && (
                <div className="mt-3 text-base text-destructive font-medium">
            ...
            <details className="mb-5">
              <summary className="text-base text-primary cursor-pointer hover:text-primary/80 font-bold py-3">
                Ročni prepis (override)
              </summary>
              <form action={overrideResultAction} className="mt-3 p-5 border-2 border-dashed border-destructive/40 rounded-xl space-y-4 bg-muted/30">
                ...
                <button
                  type="submit"
                  className="w-full sm:w-auto bg-destructive text-destructive-foreground hover:bg-destructive/90 active:bg-destructive/80 font-bold py-4 px-6 rounded-xl transition-colors text-base min-h-[52px]"
                >
                  Shrani prepis
                </button>
              </form>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {!latestResult.publishedToScoreboard ? (
                <form action={publishResultAction.bind(null, latestResult.id)} className="flex-1">
                  <button
                    type="submit"
                    className="w-full bg-accent text-accent-foreground hover:bg-accent/90 active:bg-accent/80 text-lg font-bold py-5 px-6 rounded-xl transition-colors flex items-center justify-center gap-3 min-h-[56px]"
                  >
                ...
                <form action={unpublishResultAction.bind(null, latestResult.id)} className="flex-1">
                  <button
                    type="submit"
                    className="w-full border-2 border-destructive bg-destructive/10 text-destructive hover:bg-destructive/20 active:bg-destructive/30 text-lg font-bold py-5 px-6 rounded-xl transition-colors flex items-center justify-center gap-3 min-h-[56px]"
                  >
